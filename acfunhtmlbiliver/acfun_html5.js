...
let createPlayer = function(e) {
    ...
    window.flvplayer.on('error', load_fail);
    window.flvplayer.attachMediaElement(abpinst.video);
    window.flvplayer.load();
    window.flvplayer.reloadSegment = reloadSegment;
};
...
function init() {
    ...
    playerIframe.onload = function() {
        ...
        if (localStorage.abcssText){
            playerIframe.contentDocument.head.appendChild(_('style', {
                type: 'text/css'
            }, [_('text', localStorage.abcssText)]));
        }else {
            fetch('https://raw.githubusercontent.com/zwjhuhu/TMUserScripts/master/acfunhtmlbiliver/ABPlayer.css', {
                ...
                playerIframe.contentDocument.head.appendChild(_('style', {
                    type: 'text/css'
                }, [_('text', r.responseText)]));
                localStorage.abcssText = r.responseText;
            });
        }
